...
const NotesMaker: React.FC = () => {
  ...
  const handleExportPDF = useCallback(() => {
    ...
    try {
      ...
      doc.text(`${noteType} · Generated by AI Study Hub · ${new Date().toLocaleDateString()}`, margin, y);
      y += 8;
      doc.setDrawColor(200, 200, 200); doc.line(margin, y, margin + pageW, y); y += 6;

      // Notes content — strip markdown symbols for clean PDF
      const clean = notes
      ...
      const filename = `${(topic || "notes").replace(/\s+/g, "-").toLowerCase()}-${noteType.toLowerCase()}.pdf`;
      doc.save(filename);
    } catch { /* silent fail */ }
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 min-h-[540px] relative flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {notes && (
                    <button onClick={handleExportPDF} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-slate-200 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition text-indigo-600 dark:text-indigo-400">
