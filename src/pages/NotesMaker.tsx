/**
 * NotesMaker.tsx — full dark mode support
 */
import React, { useState, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { ArrowLeft, BookOpen, Clipboard, Check, FileText, Loader2, Download } from "lucide-react";
...
const NotesMaker: React.FC = () => {
  ...
  const handleExtracted = useCallback((text: string, result: ExtractionResult) => {
    setExtractedText(text); setError(null);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault(); setError(null); setNotes("");
    ...
    try {
      ...
      let accumulated = "";
      await callAIStream(prompt, (chunk) => { accumulated += chunk; setNotes(cleanOutput(accumulated)); },
        "You are an expert academic note-taker. Respond in clean Markdown only.", 0.4, 3500);
  ...
  const handleExportPDF = useCallback(() => {
    ...
    try {
      const doc = new jsPDF();
      const margin = 15, pageW = doc.internal.pageSize.getWidth() - margin * 2, pageH = doc.internal.pageSize.getHeight();
      let y = margin;
      doc.setFontSize(16).setFont("helvetica", "bold").setTextColor(30, 30, 30);
      const titleLines = doc.splitTextToSize(topic || "Study Notes", pageW);
      doc.text(titleLines, margin, y); y += titleLines.length * 8 + 6;
      doc.setFontSize(9).setFont("helvetica", "normal").setTextColor(120, 120, 120);
      doc.text(`${noteType} · Generated by AI Study Hub · ${new Date().toLocaleDateString()}`, margin, y); y += 8;
      doc.setDrawColor(200, 200, 200); doc.line(margin, y, margin + pageW, y); y += 6;
      const clean = notes.replace(/^#{1,6}\s+/gm, "").replace(/\*\*(.*?)\*\*/g, "$1").replace(/\*(.*?)\*/g, "$1").replace(/^[-*]\s+/gm, "• ").replace(/`(.*?)`/g, "$1");
      for (const line of clean.split("\n")) {
        if (!line.trim()) { y += 4; continue; }
        const wrapped = doc.splitTextToSize(line, pageW);
        const needed = wrapped.length * 5.5 + 2;
        if (y + needed > pageH - margin) { doc.addPage(); y = margin; }
        doc.setFontSize(10).setFont("helvetica", "normal").setTextColor(50, 50, 50);
        doc.text(wrapped, margin, y); y += needed;
      }
      doc.save(`${(topic || "notes").replace(/\s+/g, "-").toLowerCase()}-${noteType.toLowerCase()}.pdf`);
    } catch { /* silent */ }
  }, [notes, topic, noteType]);

  const wordCount = notes.split(/\s+/).filter(Boolean).length;
  const readMins = Math.max(1, Math.ceil(wordCount / 200));

  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-yellow-50 dark:from-gray-950 dark:via-gray-900 dark:to-amber-950/20">
      <header className="border-b border-slate-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm sticky top-0 z-10 shadow-sm">
        <div className="container mx-auto px-5 py-3 flex items-center gap-3">
          <button onClick={() => navigate("/dashboard")} className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-800">
      ...
      <main className="container mx-auto px-5 py-10 max-w-5xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800 dark:text-white">Notes Maker</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Upload your PDF — get structured notes based only on your material.</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm p-6 space-y-5">
              <h2 className="font-bold text-slate-700 dark:text-slate-200 text-base flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 font-extrabold text-xs flex items-center justify-center">1</span>
                Configure Notes
              </h2>

              <div>
                <label htmlFor="topic" className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Topic / Focus area</label>
                <input id="topic" type="text" value={topic} onChange={e => setTopic(e.target.value)}
                  ...
                  className="w-full px-4 py-2.5 border border-slate-300 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-400 transition"
                ...
                <div className="space-y-2">
                  {NOTE_TYPES.map(t => (
                    <button key={t.value} type="button" onClick={() => setNoteType(t.value)} disabled={isLoading}
                      className={`w-full text-left px-4 py-3 rounded-xl border-2 text-sm transition-all disabled:opacity-50 ${
                        noteType === t.value
                          ? "border-amber-400 bg-amber-50 dark:bg-amber-950/50 dark:border-amber-600 shadow-sm"
                          : "border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-amber-300 dark:hover:border-amber-700"
                      }`}>
              ...
              <div>
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Upload file</p>
                <PDFUploader onExtracted={handleExtracted} onError={() => { setExtractedText(""); setExtractionInfo(""); }}
                ...
                {extractionInfo && (
                  <p className="text-xs text-green-700 dark:text-green-400 mt-1.5 flex items-center gap-1">
                    <span className="inline-block w-2 h-2 rounded-full bg-green-500" />{extractionInfo}
                  </p>
                )}
              </div>

              {error && (
                <div className="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-red-400">
          ...
          {/* Right: Output */}
          <div>
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-sm p-6 min-h-[540px] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-slate-700 dark:text-slate-200 text-base flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 font-extrabold text-xs flex items-center justify-center">2</span>
                  Generated Notes
                </h2>
                <div className="flex items-center gap-2">
                  {notes && <span className="text-xs text-slate-400 dark:text-slate-500">{wordCount} words · {readMins} min read</span>}
                  {notes && (
                    <button onClick={handleCopy} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-slate-200 dark:border-gray-600 rounded-lg hover:bg-slate-50 dark:hover:bg-gray-800 dark:text-slate-300 transition">
                      {copied ? <Check className="h-3.5 w-3.5 text-green-500" /> : <Clipboard className="h-3.5 w-3.5" />}
                      {copied ? "Copied!" : "Copy"}
                    </button>
                  )}
                  {notes && (
                    <button onClick={handleExportPDF} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-slate-200 dark:border-gray-600 rounded-lg hover:bg-slate-50 dark:hover:bg-gray-800 transition text-indigo-600 dark:text-indigo-400">
              ...
              {isLoading && !notes && (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-3">
                  ...
                  <p className="text-sm font-medium dark:text-slate-400">Generating notes from your material…</p>
                </div>
              )}
              {notes && <div className="overflow-y-auto flex-1"><MarkdownRenderer content={notes} /></div>}
              {!notes && !isLoading && (
